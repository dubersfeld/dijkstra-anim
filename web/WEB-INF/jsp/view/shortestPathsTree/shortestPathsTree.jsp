...
function canvasApp() {
	...
	function lSccSearch() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				$('#status').text('Largest SCC built');
		 		...
		 		$('#searchForm').find(':submit')[0].disabled = false;
			},
 	...
 	function searchGraph() {
 		$('#searchForm').find(':submit')[0].disabled = true;
		message = {"type":"COLLECTION"};// minimal message
				...
				graphReady = true;
			 	if (data["status"] == "OK") {
					...
					console.log("results " + results.length);
				   	$('#animForm').find(':submit')[0].disabled = false;
			 	}
	...
	function animSpeedChanged(e) {
 	...
 	function animStep() {
	  	if (animIndex < results.length) {
			...
			connGraph = jsonParse(results[animIndex]);
 	...
 	function anim() {
		console.log("anim begin");
  		$('#animForm').find(':submit')[0].disabled = true;
  		$("#largestSccForm").find(':submit')[0].disabled = true;
  		$('#searchForm').find(':submit')[0].disabled = true;
 	...
 	$("#animForm").submit(function(event) { anim(); return false; });
 	
 	$("#searchForm").submit(function(event) { searchGraph(); return false; });
...
<body>
  ...
  The strong connectedness is not a prerequisite for the Dijkstra algorithm, only a convenience choice.</p>
...
Then the Dijkstra algorithm is applied to the component, always using as source the vertex with index 0.<br/>
All intermediate results are saved as a collection that is sent back to the browser as a JSON object.<br/>
This collection is used by the browser for animation.<br/>  
...
The edges that belong to the Shortest Path Tree are colored blue, the other edges are colored black.</p>
  ...
  <div id="controls">
    <div id="SPT">
      ...
      <p>Click here to build the Shortest Paths Tree</p>
      <form name="searchForm" id="searchForm">
        <input type="submit" name="search-btn" value="Build SPT">
      </form>
      <p>Click here to start the animation</p>
      <form name="animForm" id="animForm">
        <input type="submit" name="search-btn" value="Start Animation">
      </form>
    </div>
    <div id="randomize">
